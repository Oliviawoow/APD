...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
	...
	for(int i = 0; i < nProcesses; ++i) {
		if(parents[i] == rank) {
			for(int k = 0; k < nProcesses; ++k) {
				MPI_Recv(vTopology[k], nProcesses, MPI_INT, i, 0, MPI_COMM_WORLD, &status);
	...
	if(rank != leader) {
		for(int i = 0; i < nProcesses; ++i) {
			MPI_Recv(vTopology[i], nProcesses, MPI_INT, parents[rank], 0, MPI_COMM_WORLD, &status);
			for(int j = 0; j < nProcesses; ++j) {
	...
	for(int i = 0; i < nProcesses; ++i) {
		if(parents[i] == rank) {
			for(int k = 0; k < nProcesses; k++) {
				MPI_Send(topology[k], nProcesses, MPI_INT, i, 0, MPI_COMM_WORLD);
			}
		}
	}
